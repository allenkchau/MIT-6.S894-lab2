...
void mandelbrot_cpu_vector_multicore_worker(uint32_t img_size, uint32_t max_iters, uint32_t *out, uint32_t row_begin, uint32_t row_end) {
    __m256i one = _mm256_set1_epi32(1);
    __m256 four_float = _mm256_set1_ps(4.0f);
    __m256 window_x_vec = _mm256_set1_ps(window_x);
    __m256 window_zoom_vec = _mm256_set1_ps(window_zoom);
...
void* worker_fn(void* arg) {
    ...
    uint32_t row_begin = args->row_begin;
    uint32_t row_end = args->row_end;
    mandelbrot_cpu_vector_multicore_worker(img_size, max_iters, out, row_begin, row_end);
    return nullptr;
}

// helper to get threads per core
int get_threads_per_core() {
    FILE* fp = popen("lscpu | grep 'Thread(s) per core' | awk '{print $4}'", "r");
    if (!fp) return 1;
...
void mandelbrot_cpu_vector_multicore(
    ...
    int threadsPerCore = get_threads_per_core();
    int num_threads = logical / threadsPerCore;
...
void mandelbrot_cpu_vector_multicore_multithread(
    ...
    int num_threads = std::thread::hardware_concurrency() * 10;
    int rowsPerThread = img_size / num_threads;
    int remainder = img_size % num_threads;
